...
void InterruptHandler::sys_exit()
{
    interruptManager->taskManager->ExitProcess();
}

void InterruptHandler::sys_wait(common::uint32_t pid)
{
    interruptManager->taskManager->WaitProcess(pid);
}
...
uint32_t InterruptHandler::ScheduleAgain(common::uint32_t esp)
{
    return (common::uint32_t)interruptManager->taskManager->Schedule((CPUState *)esp);
}
...
uint32_t InterruptManager::DoHandleInterrupt(uint8_t interrupt, uint32_t esp)
{
    ...
    //     printfHex(interrupt);
    // printf("\n");
    // printf("hardware interrupt offset: 0x");
    // printfHex(hardwareInterruptOffset);
    // printf("\n");
